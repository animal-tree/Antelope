...
Antelope = os.path.dirname(__file__)
app = '/'.join(str(inspect.stack()[-1][1]).split('/')[:-1])


# Imports Antelope paths and the paths of any launching app
def import_paths():
    if Antelope not in yaml_search_paths:
        yaml_search_paths.append(Antelope)  # Adds Antelope to yaml search path

    if Antelope not in module_paths:
        module_paths.append(Antelope)  # Adds Antelope to module instantiation search path
